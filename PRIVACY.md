...
- **Текст субтитров** текущего видео. В момент показа реплики её текст
  отправляется на ваш прокси-сервер (Cloudflare Worker), который вы сами
  разворачиваете, а тот — в Google Cloud Translation API для перевода.
...
1. **Ваш прокси** (Cloudflare Worker) — под вашим контролем.
